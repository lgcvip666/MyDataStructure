...
Status RecreateHashTable(HashTable *H)
{
	...
	(*H).count = 0;
	(*H).sizeindex++;
	newv = hashsize[(*H).sizeindex];	//取数组(*H).sizeindex索引处的值；
	 
	if((*H).elem!=NULL)					//elem指针变量的指向不为空，就删除指向的存储空间。
		free((*H).elem);
	
	//elem指针变量的指向为空，就扩充哈希表容量；
	(*H).elem = (KeyType *)malloc(newv * sizeof(KeyType));
	if((*H).elem==NULL)
		return ERROR;
		
	//动态申请之后，放入 NULLKEY 状态值；	
	for(i=0; i<newv; i++)
		(*H).elem[i] = NULLKEY;
